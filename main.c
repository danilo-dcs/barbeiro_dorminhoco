...
int identificadorAssento[MAX_CADEIRAS];         // troca de referencia entre cada barbeiro e cliente atrav�s de "ID's"
...
int main()
{
    	pthread_t barbeiro[NUM_BARB],cliente[MAX_CLI]; //declara��o das threads
    	int i,status=0;
    	//inicializa��o do sem�foro
    	sem_init(&clientes,0,0);
    	sem_init(&barbeiros,0,0);
    	sem_init(&mutex,0,1);
    	//inicializa��o da thread dos barbeiros

    	printf("!!A BARBEARIA ESTA ABERTA!!\n");
	for(i=0;i<NUM_BARB;i++)                     //Cria��o dos 3 barbeiros
    	{
	...
	//inicializa��o das threads dos clientes
	for(i=0;i<MAX_CLI;i++)                     //Cria��o dos clientes
	{
...
int tempoCliente()     //Gera��o de n�mero aleat�rio entre 1 e 6
{
     	int x = rand() % 6 +1; // colocar 6 no lugar do 2
	return x;
}

int tempoCorte()     //Gera��o de n�mero aleat�rio entre 3 e 6
{
     	int x = rand() % 4 +3;
	return x;
}

void threadCliente(void *tmp)  /*A��o dos Clientes*/
{
	int meuLugar, Barb;
		...
		Barb = identificadorAssento[meuLugar];    	//conex�o entre o Barbeiro e o cliente, o barbeiro atribui o ID do cliente para ele
		cadeirasLivres++;             			//levanta e vai para a cadeira do barbeiro
		sem_post(&mutex);                        	//seta o mutex de assento ocupado
		        					//clientes tendo o seu cabelo cortado pelo barbeiro 'B
	}
	else // Caso n�o tenha cadeiras Livres
	{
...
void threadBarbeiro(void *tmp)        //A��o dos Barbeiros
{
	    int indice = *(int *)(tmp);
	    int meuProximo, Cli;
	    ...
	    {
		sem_wait(&mutex);		//bloqueando o mutex para verificar se n�o h� cadeiras
		if(cadeirasLivres == MAX_CADEIRAS)  	// se n�o houver clientes
		...
		identificadorAssento[meuProximo] = pthread_self();     	//atribui o seu ID para o cliente, atrav�s do vetor de ID's
		sem_post(&mutex);
		sem_post(&clientes);         				//Chama o cliente selecionado
		        						//O Barbeiro est� cortando o cabelo do cliente 'C'
		printf("Barbeiro -%d est� acordado e cortando o cabelo do cliente-%d.\n",indice+1,Cli);
		sleep(tempoCorte());
